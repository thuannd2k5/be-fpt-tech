...
export class PaymentsService {
  ...
  async findMine(user: IUser) {
    ...
    const invoices = await this.invoiceModel.find({ enrollment_id: { $in: enrollments.map(item => item._id) } }).select('_id').lean().exec();
    return this.paymentModel.find({ invoice_id: { $in: invoices.map(item => item._id) } })
      .populate({ path: 'invoice_id', populate: { path: 'enrollment_id', populate: { path: 'class_id', populate: { path: 'course_id', select: 'name' } } } })
      .sort({ payment_date: -1 })
      .exec();
  }
