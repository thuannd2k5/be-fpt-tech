...
export class InvoicesService {
  ...
  private async createMissingInvoices() {
    ...
    const existingEnrollmentIds = new Set(
      existingInvoices.map((invoice) => String(invoice.enrollment_id)),
    );
    const missingInvoices = enrollments
      .filter(
        (enrollment) => !existingEnrollmentIds.has(String(enrollment._id)),
      )
      .map((enrollment) => {
        const classroom = enrollment.class_id as unknown as
          { course_id?: { price?: string | number } } | undefined;
    ...
    if (missingInvoices.length)
      await this.invoiceModel.insertMany(missingInvoices);
  }
  ...
  async update(updateInvoiceDto: UpdateInvoiceDto, user: IUser) {
    return await this.invoiceModel.updateOne(
      ...
      { ...updateInvoiceDto, updatedBy: { _id: user._id, email: user.email } },
    );
  }
